...
struct PetCollectionIntegrationSourceTests {
    ...
    func storeRefreshesRewardsOnASlowerScheduleAndPublishesRevealState() throws {
        ...
        #expect(source.contains("PetTrackedActivityAccumulator()"))
        #expect(source.contains("applyTrackedActivity("))
        #expect(source.contains("checkpointWithoutReward(reading)"))
        #expect(source.contains("synchronizeRewardSourceStatuses()"))
