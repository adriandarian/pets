...
struct PetOverlayTransparencyTests {
    ...
    func appUsesEachPetContextMenuInsteadOfAMenuBarExtra() throws {
        ...
        #expect(overlaySource.contains("Label(\"Hide\", systemImage: \"eye.slash\")"))
    ...
    func petContextMenuKeepsAppAndPetCommandsTogether() throws {
        ...
        #expect(overlaySource.contains("Label(\"Check for Updates…\", systemImage: \"arrow.clockwise\")"))
    ...
    func petSettingsUseAdaptiveSidebarAndDetailLayout() throws {
        ...
        #expect(detail.contains("navigation(showsTitles: true)"))
        #expect(detail.contains("navigation(showsTitles: false)"))
        #expect(detail.contains(".frame(minWidth: Self.titleVisibilityThreshold)"))
        #expect(detail.contains(".frame(minWidth: 320, maxWidth: .infinity, maxHeight: .infinity"))
        #expect(detail.contains(".layoutPriority(1)"))
        #expect(detail.contains(".frame(maxWidth: .infinity, minHeight: Self.controlHeight)"))
        #expect(detail.contains(".contentShape(Rectangle())"))
        #expect(detail.contains(".focusable()"))
        #expect(detail.contains(".focusEffectDisabled()"))
        #expect(detail.contains(".onKeyPress(.space)"))
        #expect(detail.contains(".onKeyPress(.return)"))
