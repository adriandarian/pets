...
Add a durable pet-reward loop to the native Pets settings app. Combined token usage and verified local tracker activity earn keys, keys open rarity-specific chests, and each chest unlocks a pet in a family-aware collection browser.
...
- Every **8 hours of repeatedly observed activity** across enabled activity-backed trackers awards one Common Key through a separate carried progress lane.
...
- Activity progress is recorded only while Pets is running. A provider must be active on consecutive scans, and a single scan gap can contribute at most 10 seconds so sleep or downtime cannot mint keys.
...
Enabled trackers use one of two explicit reward modes. The following sources expose trustworthy local token totals:
...
Cursor, Ollama, Gemini, Antigravity, Hermes, T3 Code, Open Design, Kiro, Zed, Windsurf, opencode, pi, NotebookLM, LM Studio, and Stitch use the activity-backed mode because they do not all expose reliable account-wide token totals. Their Collection rows show weekly tracked time, never estimated tokens. Regular apps contribute while active, interactive CLI processes contribute while running, and installed web apps contribute while their provider-specific process is detected.
...
1. **Progress header**
   ...
   - When at least one activity-backed tracker is enabled, a second progress bar shows carried tracked time toward the next 8-hour key.
   - Supporting copy keeps exact token and duration progress separate.
   - A refresh button reruns usage collection without blocking the rest of Settings.

2. **Usage sources**
   - Compact source rows show every enabled tracker. Claude, Codex, and GitHub Copilot show token totals; activity-backed trackers show weekly tracked time.
...
- carried tracked-activity seconds below eight hours;
- per-provider weekly tracked-activity totals;
...
- Eight carried hours across enabled activity-backed trackers award exactly one Common Key and retain the correct remainder.
- Disabled token trackers advance their checkpoints without rewards, so re-enabling one cannot retroactively mint keys for its disabled interval.
