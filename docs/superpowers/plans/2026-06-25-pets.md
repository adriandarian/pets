...
**Architecture:** The app is a Swift Package with a testable `PetsCore` scanner and a thin AppKit/SwiftUI overlay executable. The scanner reads `~/.claude/sessions/*.json`, filters dead PIDs, normalizes Claude statuses, and exposes sorted session models to the overlay.
...
**Files:**
- Create: `Sources/PetsCore/ClaudeSession.swift`
- Create: `Sources/PetsCore/ClaudeSessionScanner.swift`
...
**Files:**
- Create: `Sources/Pets/PetsApp.swift`
- Create: `Sources/Pets/PetStore.swift`
- Create: `Sources/Pets/PetOverlayView.swift`
...
- [ ] Run `swift build` and `swift run Pets` to manually verify the overlay launches.
